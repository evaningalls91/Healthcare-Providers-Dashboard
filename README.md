...
- Handling missing/`N/A` values (e.g. room type and admit/discharge dates for outpatient visits) so they don't distort totals.

### DAX & measures
- Writing explicit measures for the KPI cards — total billing components alongside their **per-visit averages** (e.g. `Average Amount = $674.86` vs. total `$3.4M`).
